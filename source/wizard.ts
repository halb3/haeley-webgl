...
export class Wizard {
    ...
    private static queryPrecisionSupport(
        context: Context, precision: Precision): [Precision, GLenum, GLint] {
        ...
        if (!(precision in Precision)) {
            log(LogLevel.Warning, `unknown precision '${query}' changed to '${Precision.auto}'`);
            precision = Precision.auto;
        }
        if (precision === Precision.auto) { /* Derive maximum supported write to texture/buffer format. */
            query = floatWriteSupport ? Precision.float : halfWriteSupport ?
                Precision.half : Precision.byte;
        }

        /* Query type and, if required), enable extension. */
        if (query === Precision.half && halfWriteSupport) {
            context.isWebGL2 ? context.colorBufferFloat : context.textureHalfFloat;
            return [query, gl2facade.HALF_FLOAT, 1];

        } else if ((query === Precision.float || query === Precision.half)
            && floatWriteSupport) {
    ...
    static queryInternalTextureFormat(context: Context, target: GLenum,
        precision: Precision = Precision.auto): [GLenum, GLenum, Precision] {

...
import { BackendType } from '../source/context';
...
describe('ExtensionsHash', () => {
    ...
    it('should encode version and backend in head', () => {
        // version 0b000
        expect(ExtensionsHash.encode(BackendType.WebGL1, [])[0]).to.equal('1');
        expect(ExtensionsHash.encode(BackendType.WebGL2, [])[0]).to.equal('2');
    });

    it('should encode single extensions', () => {
        expect(ExtensionsHash.encode(BackendType.WebGL1,
